...
workflow {
  ch_fastq = Channel.fromFilePairs( "${params.input}/Data/Intensities/BaseCalls/*_{R1,R2}_*.fastq.gz" )
  ch_sample_sheet = Channel.fromPath( "${params.input}/SampleSheet.csv" )
  ch_multiqc_config = Channel.fromPath( "${projectDir}/assets/multiqc_config_base.yaml" )
  ch_run_dir = Channel.fromPath(params.input)
  ch_run_id = Channel.fromPath(params.input).map{ it -> it.baseName }
  ...
  main:
    ...
    abundance_top_n.out.filter{ it[2] == 'Genus' }.map{ it -> it[1] }.collectFile(keepHeader: true, sort: { it.text }, name: "top_3_abundances_genus.csv", storeDir: "${params.output}/RoutineQC/abundance_top_n")
    abundance_top_n.out.filter{ it[2] == 'Species' }.map{ it -> it[1] }.collectFile(keepHeader: true, sort: { it.text }, name: "top_5_abundances_species.csv", storeDir: "${params.output}/RoutineQC/abundance_top_n")
